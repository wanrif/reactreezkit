...
function preventScrollMobileSafari() {
  ...
  const onTouchStart = (e: Event) => {
    ...
    scrollable = getScrollParent(touchEvent.target as Element)
    if (
  ...
  const onTouchMove = (e: Event) => {
    ...
    const y = touchEvent.changedTouches[0].pageY
...
function setStyle(
  ...
  const cur = (element.style as unknown as Record<string, string>)[
    style as string
  ]
  ;(element.style as unknown as Record<string, string>)[style as string] = value

  return () => {
    ;(element.style as unknown as Record<string, string>)[style as string] = cur
  }
}
